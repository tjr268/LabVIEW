...
%Simulate Object
Image = imread('questionmark1.jpg','bmp'); %imports image Question mark is a BMP!
...
Objwave = abs(Image).*exp(1i.*phi) + (0.01*lambda)*rand(256);
figure(2); 
...
y = 1:1:Y; %makes array
[kx,ky] = meshgrid(x,y);
Referencewave = abs(Image).*exp(1i*(kx + ky)) + (0.01*lambda)*rand(256);
...
k = 2*pi/lambda;
E = exp(1i*z*sqrt((k)^.2 - (kx.*ky) - (ky.*ky)));
